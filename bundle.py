...
def run(run_index, iteration_index, input, output, user_programs):
    SCORE = 0
    xml = user_programs[0]
    return {'score': SCORE, 'message': 'Success', 'log': get_log()}
...
def create_state_machine_from_xml(xml_content: str, parameters: dict = None) -> StateMachine:
    """
    Convenience function to create a StateMachine from XML content.
    
    Args:
        xml_content: CGML XML content as string
        parameters: Parameters for the state machine components
    ...
    if not elements.state_machines:
        raise ValueError("No state machines found in XML")
    
    state_machine_id = list(elements.state_machines.keys())[0]
    cgml_sm = elements.state_machines[state_machine_id]
    
    return StateMachine(cgml_sm, parameters)


def run_simple_test(xml_content: str, signals: list[str], parameters: dict = None, timeout: float = 10.0) -> StateMachineResult:
    """
    Convenience function to run a simple test with XML content and signals.
    
    Args:
        xml_content: CGML XML content as string
        signals: List of signals to send to the state machine
        parameters: Parameters for the state machine components
    ...
    Returns:
        StateMachineResult
    """
    sm = create_state_machine_from_xml(xml_content, parameters)
    return run_state_machine(sm, signals, timeout)
